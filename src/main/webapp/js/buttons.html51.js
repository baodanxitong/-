...
{
	var $index = $.inArray($(obj).val(), columlist);
	if($index != -1)
	{
		columlist.splice(jQuery.inArray($(obj).val(),columlist),1); 
	}
	else{
		columlist.push($(obj).val());
	}
	//alert(columlist);
}
(function( factory ){
...
function newDataHandle($arrres) {
	
	var $filterArray = columlist;
	var $newdata =  Array();
	
	
	$.each($arrres,function(i,k){
		var temp_k = Array();
...
function newHeaderHandle($arrres) {
...
DataTable.ext.buttons.excelHtml5 = {
	...
	action: function ( e, dt, button, config ) {
		...
		if(columlist.length>0){
			var newbody = newDataHandle(data.body);
			var newheader = newHeaderHandle(data.header);
			data.body = newbody;
